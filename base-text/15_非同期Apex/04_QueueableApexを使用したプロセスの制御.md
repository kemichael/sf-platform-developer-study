...
この単元では、future メソッドの上位互換（スーパーセット）である「Queueable Apex」の構文・3 つの利点・ジョブのチェーニングを学びました。

次の表は、future と Queueable の違いを「何が解決されるか」の視点で凝縮したものです。

| 比較項目 | future メソッド | Queueable Apex |
| --- | --- | --- |
| 宣言方法 | メソッドに `@future` | クラスに `Queueable` を実装 |
| 引数・メンバー | プリミティブ型のみ | **sObject など非プリミティブ型も可** |
| ジョブ ID の取得 | できない | **`enqueueJob()` が ID を返す（監視可）** |
| ジョブのチェーニング | できない | **できる（1 親 → 1 子）** |
| 向いている場面 | 同期/非同期を切り替えたい | 連鎖・複雑な型・監視が必要 |

> [!まとめ] この単元の要点
>
> - 実装は **`Queueable` インターフェース**、メソッドは **`execute(QueueableContext context)`**、登録は **`System.enqueueJob()`**。
> - future に対する 3 つの利点：**①非プリミティブ型（sObject）OK ②ジョブ ID が返る（監視可）③チェーニング可**。
> - 1 トランザクションで enqueue できるのは最大 **50** ジョブ。チェーニングは **1 親 → 1 子**（分岐不可）。
> - **Developer Edition / トライアル組織**のチェーン最大スタック深度は **5**（親含め最大 5 ジョブ）。
> - テストは `Test.startTest()` / `Test.stopTest()` で囲み、`stopTest()` 後に同期実行される。

> [!豆知識] 「迷ったら Queueable」が現場の合言葉
>
> Salesforce 自身が「future より Queueable を推奨」と公式に述べており、新規開発では future をあえて選ぶ理由はかなり限られます（同期メソッドを軽くラップしたいときくらい）。クラスとして書けるためコンストラクターで状態を持て、テストもしやすく、監視も連鎖もできる——「非同期で迷ったらまず Queueable」と覚えておくと設計判断がぶれません。
