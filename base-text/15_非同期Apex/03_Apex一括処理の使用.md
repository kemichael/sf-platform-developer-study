...
この単元では、数千〜数百万件の大量レコードを 200 件ずつのバッチに分け、各バッチを別トランザクションとして安全に処理する「Apex 一括処理」の 3 メソッド構造と状態保持を学びました。

次の図は、`Database.Batchable` の 3 メソッドがジョブのライフサイクルでどう呼ばれるかを俯瞰したものです。

```mermaid
flowchart TD
    S(["大量レコード<br/>（数千〜数百万件）"]) --> ST["start()<br/>対象を収集・1回だけ<br/>QueryLocator は最大5000万件"]
    ST --> SP{"200件ずつのバッチに分割<br/>（scope で変更可）"}
    SP --> EX["execute()<br/>バッチごとに複数回<br/>各バッチは別トランザクション"]
    EX --> FN["finish()<br/>全バッチ完了後に1回<br/>メール送信など後処理"]
    FN --> E(["ジョブ完了"])
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class S hl;
    class ST,EX,FN,E soft;
```

> [!まとめ] この単元の要点
>
> - 実装は **`Database.Batchable<sObject>`**、必須は **`start()` / `execute()` / `finish()`** の 3 メソッド。
> - 呼び出しは **`Database.executeBatch(インスタンス [, scope])`**。`scope` 省略時のバッチサイズは **200**。
> - **各バッチは独立したトランザクション**で新しいガバナ制限のセットで開始。1 バッチが失敗しても他はロールバックされない。
> - **`QueryLocator` は最大 5,000 万件**（SOQL 行数制限が無視される）。`Iterable` は通常の制限が適用。
> - バッチをまたいで値を保持したいなら **`Database.Stateful`**（インスタンス変数のみ保持）。
> - **テストでは 1 バッチ分（200 件以下）のみ**実行できる。

> [!豆知識] バッチサイズは「速さ」ではなく「安全」のためのつまみ
>
> バッチサイズ（scope）を小さくしても処理が速くなるわけではありません。むしろバッチ数が増えてオーバーヘッドは増えます。scope を下げる本当の目的は「1 トランザクションあたりの負荷を下げてガバナ制限超過を避けること」。重い `execute()` で制限に近づくときだけ 100 や 50 に絞る、という使い方が正解です。
