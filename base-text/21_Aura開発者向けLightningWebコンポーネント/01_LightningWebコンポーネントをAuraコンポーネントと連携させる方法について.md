...
この単元では、Salesforce の 2 つのコンポーネント開発モデル（Aura と LWC）の関係、LWC の利点、共存ルール、そして Aura から LWC への移行を検討すべきタイミングを学びました。

次の図は、新規・既存コンポーネントをどのモデルで扱うかを判断する流れを俯瞰したものです。

```mermaid
flowchart TD
    S(["コンポーネントをどうする？"]) --> Q1{"新規作成か<br/>既存の見直しか？"}
    Q1 -->|"新規作成"| NEW["LWC で作るのが推奨<br/>（Web 標準・高速）"]
    Q1 -->|"既存 Aura"| Q2{"UI のみ／表示が遅い／<br/>大規模ツリーか？"}
    Q2 -->|"はい"| MIG["LWC へ移行を検討<br/>（設計を見直す好機）"]
    Q2 -->|"いいえ"| KEEP["当面は Aura を維持"]
    NEW --> CO{"組み合わせる？"}
    MIG --> CO
    KEEP --> CO
    CO -->|"共存ルール"| RULE["Aura に LWC は OK<br/>LWC に Aura は NG"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class S hl;
    class NEW,MIG,RULE soft;
```

> [!まとめ] この単元の要点
>
> - Salesforce には **Aura**（従来）と **LWC**（新・Web 標準）の 2 つのプログラミングモデルがある。
> - LWC の 3 大利点は **最新の JavaScript / 高い生産性 / 高速なパフォーマンス**。
> - 共存は一方向ルール：**Aura に LWC は含められる ✅ / LWC に Aura は含められない ❌**。外側が LWC なら配下はすべて LWC。
> - 移行は 1 行変換ではなく **設計の見直し**。候補は **UI のみ／パフォーマンス重視／大規模ツリー**。
> - **ES6 モジュール**で共通コードを書けば Aura からも LWC からも再利用できる。

> [!豆知識] LWC は「2019 年デビュー」の比較的新しいモデル
>
> Lightning Web Components が一般提供（GA）されたのは 2019 年の Spring '19 リリースです。Aura（2014 年頃〜）から数えて約 5 年後の登場でした。LWC が「最新の JavaScript」を売りにできるのは、ブラウザーが Web コンポーネント標準（カスタム要素・Shadow DOM など）にネイティブ対応した時代に設計されたためです。Aura は標準が固まる前に作られたため、当時は独自フレームワークで足りない部分を補う必要がありました。
