...
この単元では、Aura の独自イベントモデル（コンポーネントイベント／アプリケーションイベント）が LWC では Web 標準の DOM イベントと pub-sub パターンに置き換わること、そして LWC のイベントを Aura ラッパーが捕まえて橋渡しする「catch and release」を学びました。

次の図は、イベントが子から親へ伝わり、さらに兄弟や Aura へ橋渡しされる経路を俯瞰したものです。

```mermaid
flowchart TD
    Child["子 LWC<br/>dispatchEvent（CustomEvent）"] -->|"DOM イベント<br/>（上へバブリング）"| Parent["親 LWC<br/>onxxx={handler} で受信"]
    Parent -->|"兄弟へ届けたい"| PubSub["pub-sub<br/>register / fire / unregister"]
    Child -->|"Aura ラッパーが囲む場合"| Aura["Aura が catch<br/>evt.getParam で取得"]
    Aura -->|"必要なら"| Release["Aura イベントを起動<br/>（release）"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class Child hl;
    class Parent,PubSub,Aura soft;
```

> [!まとめ] この単元の要点
>
> - LWC は独自イベントモデルを捨て、**Web 標準の DOM イベント**を使う。発火は **`this.dispatchEvent(new CustomEvent('名前', { detail: データ }))`**。
> - Aura の **コンポーネントイベント → DOM イベント**、**アプリケーションイベント → pub-sub パターン**（`register`/`unregister`/`fire`）。
> - 処理は **`on` プレフィックス**の宣言型ハンドラー（`selected` → `onselected`）。イベント名は**小文字**。
> - データは `CustomEvent` の **`detail`**。LWC 側は **`event.detail`**、Aura ラッパー側は **`evt.getParam()`** で取り出す。
> - 通信の向き：**子 → 親はイベント**、**親 → 子は子の `@api` メソッド**。Aura は LWC の DOM イベントを**捕まえて（catch）放つ（release）**橋渡しができる。

> [!豆知識] pub-sub の弱点を解消する「Lightning Message Service」
>
> この単元の `pubsub.js` は便利ですが、同じ Lightning ページ内でしか通信できず、コピーして使う「お手製」モジュールです。Salesforce はその後、ページをまたいで（さらに Aura・LWC・Visualforce の壁も越えて）メッセージをやり取りできる公式機能 **Lightning Message Service（LMS）** を導入しました。新しく兄弟間通信を設計するなら、自前の pub-sub よりも LMS を検討するのが現在の推奨です。試験範囲としては pub-sub の `register`/`unregister`/`fire` を押さえつつ、実務では LMS という選択肢があると覚えておくとよいでしょう。
