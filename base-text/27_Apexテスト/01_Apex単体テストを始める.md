...
```text
getTaskPriority('NY') だけをテストした場合のカバー状況
──────────────────────────────────────────────
 if(isBlank || length>2)   ← 通った（NYは有効）
   return null;            ← ✗ 未カバー（無効入力を渡していない）
 if(leadState == 'CA')     ← 通った（false 側のみ）
   taskPriority='High';    ← ✗ 未カバー（CAを渡していない）
 else
   taskPriority='Normal';  ← 通った
 return taskPriority;      ← 通った
──────────────────────────────────────────────
 結果: 6行カバー / 2行未カバー → 75%
```
