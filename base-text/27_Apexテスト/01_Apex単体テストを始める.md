...
この単元では、Apex コードが正しく動くことを別コードで自動検証する「単体テスト」の書き方と、リリースに必須となる 75% カバー率の考え方を学びました。

次の図は、テストクラスを書いてから実行し、カバー率を確認してリリースに至るまでの流れを俯瞰したものです。

```mermaid
flowchart TD
    S(["@IsTest クラス・メソッドを書く"]) --> A["入力を準備し対象メソッドを実行"]
    A --> V["Assert.areEqual などで期待値を検証"]
    V --> R["テストを実行（Run All 推奨）"]
    R --> Q{"全テスト合格 かつ<br/>カバー率 75% 以上？"}
    Q -->|"いいえ"| FIX["失敗修正・分岐ごとにテスト追加"]
    FIX --> A
    Q -->|"はい"| OK(["リリース・パッケージ化が可能"])
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class S hl;
    class A,V,R,FIX,OK soft;
```

> [!まとめ] この単元の要点
>
> - テストはクラスとメソッドの**両方**に `@IsTest` を付け、メソッドは **static** で定義する（単体テスト専用クラスは private）。
> - 本番リリース・パッケージ化には**組織全体で 75% 以上**のカバー率と、**全テスト合格**、**各トリガー1件以上のカバー**が必須。
> - 検証は `Assert` クラスを使い、`Assert.areEqual(期待値, 実際の値)` の**引数の順番**を守る。
> - 100% を目指すには `if` 分岐の**両側の経路**をそれぞれ通すテストを書く（分岐カバレッジ）。
> - テストで作ったレコードは終了時に**ロールバック**され、メール送信・コールアウト・SOSL は実行されず、組織の既存データには原則アクセスできない。

> [!豆知識] 75% という数字の由来
>
> 「なぜ 80% でも 70% でもなく 75% なのか」と疑問に思う人は多いですが、これは Salesforce が定めた最低ラインです。実務では品質確保のため 85〜90% を社内基準にするチームも珍しくありません。なお `@IsTest` で囲んだテストコード自体はカバー率の分母（対象行）にカウントされないため、テストを増やしても「テストでカバー率を水増しする」ことはできない仕組みになっています。
