...
```text
  ① アクションを作成する
       Agentforce Studio で Apex の @InvocableMethod を
       「カスタムアクション」として登録
              │
              ▼
  ② サブエージェントに割り当てて確定・有効化する
       Answer Weather Questions サブエージェントに追加
...
>           │
>           ▼
>   [サブエージェントに移行] Answer_Weather_Questions へ
>           │
>           ▼
>   [推論] 天気の質問 → Check Weather アクションを選択
>           │
>           ▼
>   Apex（CheckWeather → WeatherService）を実行し気温を取得
>           │
>           ▼
>   気温データをもとに回答文を生成してユーザーに返す
> ```
