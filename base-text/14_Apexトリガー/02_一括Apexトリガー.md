...
この単元では、トリガーが常に複数件（最大 200 件のバッチ）をまとめて処理する前提でコードを書く「一括化（Bulkification）」を学びました。柱は「`for` ループで全件処理」と「SOQL / DML はループの外で1回だけ」の2つです。

次の表は、一括化していないアンチパターンと、一括化した正しいパターンの対比です。

| 観点 | アンチパターン（NG） | ベストプラクティス（OK） |
| --- | --- | --- |
| レコードの扱い | `Trigger.new[0]` だけ処理 | `for` ループで全件処理 |
| SOQL の位置 | ループの**中**で発行（件数分） | ループの**外**で1回（バインド変数） |
| DML の位置 | ループの**中**で実行（件数分） | リストに溜めてループ**外**で1回 |
| 結果 | 101 件目で `Too many SOQL/DML` 例外 | 200 件超でもガバナ制限に余裕 |

> [!まとめ] この単元の要点
>
> - 一括化とは「複数件をまとめて正しく・効率的に処理する」書き方。柱は (1) `for` ループで全件処理、(2) SOQL / DML はループの外で1回だけ。
> - SOQL をループ内に書くと `Too many SOQL queries: 101`、DML をループ内に書くと `Too many DML statements: 151` で失敗する。
> - 関連レコードは**バインド変数**（`WHERE Id IN :Trigger.new`）で1クエリ取得、更新対象は**リストに溜めて1回 DML**。
> - トリガーは **200 件ずつのバッチ**で起動し、ガバナ制限はバッチ単位でカウントされる。
> - `Trigger.operationType` + `switch` で挿入/更新を分け、無駄なクエリ（挿入時の商談チェック等）を省ける。

> [!豆知識] なぜ「200 件」なのか
>
> トリガーが一度に処理するバッチサイズが 200 件なのは、Salesforce が API・データローダー経由の DML を内部的に 200 件ずつのチャンクに分割するためです。データローダーの既定バッチサイズも 200。だから「合計 400 件」でも 200 件 × 2 回の独立したトリガー起動になり、ガバナ制限（SOQL 100 回など）はそれぞれの起動でリセットされます。「件数が増えても制限が比例して厳しくなるわけではない」のはこの仕組みのおかげです。
