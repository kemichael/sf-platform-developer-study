...
この単元では、2人の開発者（Ella と Juan）が「ブランチ作成 → サンドボックス承認 → 開発 → retrieve/deploy → コミット → PR → マージ」という組織開発モデルの基本ワークフローを一巡しました。

次の表は、Ella と Juan の作業を「作り方」「向き（retrieve/deploy）」「使うフラグ」で対比したものです。同じワークフローでも、宣言型 UI で作ったか・コードで書いたかで取得とリリースの向きが入れ替わる点が核心です。

| 開発者 | 作り方 | 成果物 | 組織との向き | 使うフラグ |
| --- | --- | --- | --- | --- |
| **Ella** | 設定 UI（宣言型）でオブジェクト・項目を作成 | CustomObject / CustomField | **retrieve**（組織 → ローカル） | `--metadata`（新規取得） |
| **Juan** | VS Code でトリガー・テストをコーディング | ApexTrigger / ApexClass | **deploy**（ローカル → 組織） | `--source-dir`（既存ファイル） |

> [!まとめ] この単元の要点
>
> - プロジェクト開始時は、まず **DX プロジェクト作成（またはクローン）→ 自分のブランチ作成**。変更前の必須手順。
> - **retrieve は組織 → ローカル（取得）**、**deploy はローカル → 組織（リリース）**。矢印の向きを逆に覚えない。
> - フラグは `--metadata`（型と名前で指定・新規取得向き）と `--source-dir`（既存ファイルのパス指定）を使い分ける。
> - ログイン URL は **サンドボックス = `test.salesforce.com`、本番/DE/Playground = `login.salesforce.com`**。
> - 変更共有は「Sandbox → ソースリポジトリへ取り込み → コミット → PR → マージ」。各メンバーの Sandbox に直接配らない。Apex には**カバー率 75% 以上**のテストが必須。

> [!豆知識] `--source-dir` で新規メタデータが取れない理由
>
> `--source-dir` はローカルにすでにある「ファイルのパス」を指定する方式のため、まだ手元に存在しない新規メタデータは対象にできません。だから Ella は初回取得で `--metadata`（型と名前で指定）を使いました。「初めて取ってくるものは `--metadata`、手元にあるものは `--source-dir`」と覚えると迷いません。
