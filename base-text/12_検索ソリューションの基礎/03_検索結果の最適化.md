...
この単元では、SOSL の絞り込み句（`IN` / `RETURNING(...)` / `ORDER BY` / `LIMIT` / `OFFSET` / `WITH`）で検索を効率化する方法、推奨結果を返す REST リソース、そして管理者によるシノニムグループ・昇格済み検索語での結果強化を学びました。

次の表は、効率的な検索を「対象を絞る／件数を絞る／見せ方を整える」の 3 つの役割で整理したものです。

| 役割 | SOSL の句 | はたらき |
| --- | --- | --- |
| 検索対象を絞る | `IN SearchGroup` | 名前・メールなど項目の種類を限定 |
| 検索対象を絞る | `WITH`（DIVISION / DATA CATEGORY など） | ディビジョン・分類などで限定 |
| 返す件数を絞る | `LIMIT n` / `OFFSET n` | 最大件数の指定・ページ送り |
| 見せ方を整える | `ORDER BY` | 結果の並び替え |

> [!まとめ] この単元の要点
>
> - 効率的な検索は「**検索対象を絞る**（`IN` / `WITH`）」と「**返す件数を絞る**（`LIMIT` / `OFFSET`）」の 2 軸で組み立てる。
> - `RETURNING FieldSpec` の中に `FieldList` / `ORDER BY` / `LIMIT` / `OFFSET` をまとめて書ける。`OFFSET` は上限（一般に 2,000）に注意。
> - 句の対応：SOSL の `IN`↔SOQL の `WHERE`、SOSL の `RETURNING`↔SOQL の `SELECT`。`ORDER BY` / `LIMIT` / `OFFSET` / `WITH DATA CATEGORY` はほぼ共通。
> - **推奨結果（typeahead）** は REST リソース（Search Suggested Records / Article Title Matches / SObject Suggested Articles for Case）で取得する。
> - ランキング強化は**管理者の領域**。**シノニムグループ**（2〜6 語）と**昇格済み検索語**（最大 2,000 個・やりすぎ注意）を設定する。

> [!豆知識] 昇格済み検索語は「やりすぎ厳禁」の調整つまみ
>
> 昇格済み検索語は特定キーワードで記事を最上部に固定できる便利機能ですが、登録しすぎると Salesforce の関連性ランキングそのものをゆがめ、かえって検索体験が悪化します。組織で最大 2,000 個まで作れますが、「本当にトップに出したい数記事だけ」に絞るのがプロの使い方。検索チューニングは足し算より引き算が効く、という良い例です。
