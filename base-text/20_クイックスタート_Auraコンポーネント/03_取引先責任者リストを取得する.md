...
この単元では、JavaScript コントローラーから **値プロバイダー `c.`** で Apex メソッドを呼び出し、結果を非同期のコールバックで属性に保存する一連の手順と、`init` イベントで読み込み時に自動起動する仕組みを学びました。

次の図は、Aura でサーバー側 Apex を呼び出すときの「定番4ステップ」を俯瞰したものです。

```mermaid
flowchart LR
    G["component.get<br/>（c.getContacts）<br/>① アクション取得"] --> P["setParams<br/>② 引数 recordId を設定"]
    P --> CB["setCallback<br/>③ 結果処理を登録"]
    CB --> EQ["$A.enqueueAction<br/>④ キューに入れ送信"]
    EQ --> R(["コールバックで<br/>v.Contacts に保存"])
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class EQ hl;
    class G,P,CB,R soft;
...
> [!まとめ] この単元の要点
>
> - サーバー呼び出しの定番手順は **`component.get("c.メソッド")` → `setParams` → `setCallback` → `$A.enqueueAction`**。
> - **`$A.enqueueAction` を呼ばないとアクションは送信されない**（忘れやすい頻出ポイント）。
> - サーバー処理は **非同期**。結果は必ず `setCallback` の中で `data.getReturnValue()` で受け取る。
> - データ参照は **`v.`**、メソッド参照は **`c.`**。取り違えると動かない。
> - **`aura:handler` の `init` イベント**で、読み込み時に処理を自動起動できる。

> [!豆知識] enqueueAction は「まとめて送る」ための仕組み
>
> `$A.enqueueAction` の "enqueue"（キューに入れる）という名前には理由があります。Aura は呼び出しを即送信せず一度キューに溜め、イベント処理の区切りで **複数のサーバー要求を1回の通信にまとめて（バッチ化して）送信**します。これにより往復回数が減り、画面の応答性が上がります。「すぐ送らずまとめて送る」のが Aura のサーバー通信の基本設計です。
