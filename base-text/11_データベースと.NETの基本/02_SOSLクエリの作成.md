...
この単元では、複数オブジェクトを横断してテキストを全文検索する **SOSL（`FIND` で始まる検索言語）** を学びました。インデックスの管理は不要（全自動）で、SOQL との使い分けが試験の中心です。

次の表で、SOSL の3部構成と SOQL との違いを総括します。

| 観点 | 内容 |
| --- | --- |
| 構文 | `FIND {検索語}` → `IN 検索グループ` → `RETURNING オブジェクト(項目)` |
| 必須/任意 | `FIND` が必須、`IN`・`RETURNING` は任意 |
| 検索語の囲み | クエリエディターは中括弧 `{}`、Apex は一重引用符 `''` |
| ワイルドカード | `*`（0文字以上）／ `?`（ちょうど1文字） |
| ニックネーム検索 | 取引先・取引先責任者・リード・ユーザーの**英語のみ** |
| 使い分け | 無関係な複数オブジェクトの横断テキスト検索 → SOSL／条件が明確な単一照会 → SOQL |

> [!まとめ] この単元の要点
>
> - SOSL は **`FIND` で始まる全文検索言語**。SOQL（`SELECT`）とは別物。
> - 構文は **`FIND` → `IN` → `RETURNING`** の3部構成。`FIND` のみ必須。
> - **Lucene** ベースだが、インストール・設定・**インデックス管理は不要**（全自動・非同期）。
> - 検索語の囲みは**クエリエディター=中括弧 `{}`／Apex=一重引用符 `''`**。
> - **複数の（関連していない）オブジェクトを横断してテキストを探す → SOSL**、条件が明確な単一オブジェクト → SOQL。

> [!豆知識] 「joey」で「Joseph」が見つかる仕組み
>
> SOSL のシノニム（ニックネーム）検索は、Joseph ⇄ Joey、Robert ⇄ Bob、William ⇄ Bill のような英語圏の愛称辞書を内部に持っています。これが効くのは取引先・取引先責任者・リード・ユーザーの英語データのみで、アップロードしたドキュメントやカスタムオブジェクトには効きません。だから本文の例では `joey` ではヒットしなかった `TestDocument` が、前方一致の `jos*` では拾えたのです。
