...
この単元では、Salesforce DX が「ソースコード中心の開発」を実現するツールセットであることを学び、その土台となる「Dev Hub の有効化 → CLI のインストール → CLI からの認証」という3ステップを通しで押さえました。

次の表は、DX 開発を始めるための登場人物と、この単元で行ったセットアップ作業を整理したものです。

| 登場人物 / 作業 | 役割・意味 | この単元での操作 |
| --- | --- | --- |
| **Salesforce DX** | ソース駆動の開発を行うツールセット | 概念を理解する |
| **Dev Hub** | スクラッチ組織を発行・管理する司令塔 | Trailhead Playground で有効化 |
| **Salesforce CLI（`sf`）** | コマンドで組織を操作する道具 | インストールし `sf update` で確認 |
| **認証（login web）** | CLI と Dev Hub を結びつける | `sf org login web -d -a DevHub` |

> [!まとめ] この単元の要点
>
> - **Salesforce DX** はソースコードを VCS で管理し、CLI で組織へ反映する「ソース駆動」の開発スタイル。
> - **Dev Hub** はスクラッチ組織の発行元（工場）、**スクラッチ組織**は使い捨ての開発環境という役割の違いを区別する。
> - 開発は本番組織ではなく **Developer Edition / Trailhead Playground** で行うのが鉄則。
> - セットアップは **Dev Hub 有効化 → CLI インストール（`sf update`）→ CLI 認証（`sf org login web -d -a DevHub`）** の順。
> - フラグ `-a` は別名（エイリアス）、`-d` はデフォルト（既定の Dev Hub）指定。

> [!豆知識] `sfdx` から `sf` へ：コマンド体系の世代交代
>
> 初期の Salesforce CLI は `sfdx force:org:create ...` のようにコロン区切りの長いコマンドでした。現行の `sf` 体系では `sf org create scratch ...` のように「名詞 → 動詞」の自然な並びへ刷新され、入力もぐっと短くなっています。古い記事で `sfdx` を見かけたら「旧世代の書き方」と読み替えると混乱しません。
