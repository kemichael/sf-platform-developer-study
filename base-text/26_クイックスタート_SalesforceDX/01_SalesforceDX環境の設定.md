...
```text
   ┌──────────────────────────────┐
   │   開発者ハブ（Dev Hub）        │  ← 1つのメイン組織
   │   スクラッチ組織を発行・管理     │
   └───────────────┬──────────────┘
                   │ 発行（create）
        ┌──────────┼──────────┐
        ▼          ▼          ▼
   スクラッチ組織A  組織B      組織C   ← 短期・使い捨ての開発環境
   （機能Xの開発）  （テスト）  （実験）
```
...
```text
 ① Dev Hub を有効化       ② CLI をインストール       ③ CLI から Dev Hub に認証
 （Trailhead Playground）   （sf update で確認）        （sf org login web -d -a DevHub）
        │                        │                          │
        ▼                        ▼                          ▼
 スクラッチ組織を作れる   コマンドで組織を操作可能   CLI が Dev Hub を記憶
        └────────────────────────┴──────────────────────────┘
                                  ▼
                    次の単元へ：プロジェクトの設定 → スクラッチ組織の作成
```
