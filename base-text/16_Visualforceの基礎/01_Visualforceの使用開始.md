...
この単元では、Visualforce が「Salesforce 上で独自の UI を作る Web 開発フレームワーク」であること、その処理がサーバー側で実行されて HTML として返ること、そして 7 つの表示場所を学びました。

次の図は、Visualforce の「正体・処理の流れ・表示できる場所」を 1 枚で俯瞰したものです。

```mermaid
flowchart TD
    VF["Visualforce<br/>カスタム UI を作るフレームワーク"] --> SV["処理はサーバー側で実行<br/>HTML に変換して返す"]
    VF --> CT{"コントローラーの選択"}
    CT -->|"コード不要"| STD["標準コントローラー<br/>データ取得・保存"]
    CT -->|"複雑な処理"| APX["Apex<br/>カスタムコントローラー"]
    VF --> PL["表示できる7つの場所<br/>ランチャー・ナビバー・<br/>ページレイアウト など"]
    PL -.->|"表示できない"| NG["[設定] ページの中<br/>（ひっかけ注意）"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    classDef ng fill:#FCE8E8,stroke:#C23934,color:#5A1A1A;
    class VF hl;
    class SV,CT,STD,APX,PL soft;
    class NG ng;
```

> [!まとめ] この単元の要点
>
> - Visualforce は **カスタム UI を作る Web 開発フレームワーク**。処理は **サーバー側**で実行され、**HTML に変換**されてブラウザーに返る。
> - **標準コントローラー**でコードなしのデータアクセス・保存ができ、複雑な処理は **Apex（カスタムコントローラー）**で補う。
> - 構成要素は **マークアップ（`<apex:～>`）・コントローラー・約 150 個の組み込みコンポーネント**。
> - 表示できる場所は **7 つ**（ランチャー / ナビゲーションバー / ページレイアウト / Lightning アプリケーションビルダー / クイックアクション / ボタン・リンクの上書き / カスタムボタン・リンク）。**[設定] ページの中には表示できない**。
> - 現在の UI 構築の推奨は **LWC** だが、Visualforce も PDF 生成・メールテンプレート・ページ埋め込みなどで現役。

> [!豆知識] 名前の由来は「目に見えるフォース」
>
> Visualforce の「Force」は、かつての開発者向けプラットフォーム名「Force.com」に由来します。「目に見える（Visual）形で Force プラットフォームの力を引き出す」という意味合いで、2008 年の登場以来、Lightning Web コンポーネントが主役になった今もサポートが続く長寿フレームワークです。
