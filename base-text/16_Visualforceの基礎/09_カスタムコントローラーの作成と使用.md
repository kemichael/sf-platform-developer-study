...
>  ┌──────────────────────────────┐
>  │ Visualforce ページ             │
>  │  controller="ContactsList…"   │──┐ 結び付け
>  │  {! contacts } などの式        │  │
>  └──────────────────────────────┘  │
>                                     ▼
>  ┌──────────────────────────────┐
>  │ Apex クラス（カスタムコントローラー）│
>  │  getContacts() / アクション …  │
>  └──────────────────────────────┘
> ```
>
...
>
> ```text
>  [First Name] ヘッダーをクリック
>        │
>        ▼
>  sortByFirstName() 実行 → sortOrder = 'FirstName'
>        │
>        ▼
>  reRender="contacts_list" で表を部分再表示
>        │
>        ▼
>  {! contacts } 再評価 → getContacts() が新しい並び順でクエリ
>        │
>        ▼
>  名前順に並び替わった表が表示される
> ```
