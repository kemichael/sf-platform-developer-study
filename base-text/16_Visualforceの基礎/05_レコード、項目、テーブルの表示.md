...
この単元では、参照のみの UI を作る **出力コンポーネント**を、大まか／きめ細かい／反復という粒度の違いで使い分ける方法を学びました。

次の表は、4 つの主役コンポーネントの粒度と役割を凝縮したものです。

| コンポーネント | 粒度 | 役割 |
| --- | --- | --- |
| `<apex:detail>` | 大まか | 詳細＋関連リストを丸ごと再現（最速） |
| `<apex:relatedList>` | 大まか（下位） | 関連リストを 1 つだけ個別に表示 |
| `<apex:outputField>` | きめ細かい | 1 項目を**型に応じた書式付き**で表示 |
| `<apex:pageBlockTable>` | 反復 | `value`＝リスト、`var`＝1 件ずつの変数で表を生成 |

> [!まとめ] この単元の要点
>
> - **大まか**（`<apex:detail>`）は少ない記述で多機能、**きめ細かい**（`<apex:outputField>`）は組み合わせて細部まで設計、と粒度で使い分ける。
> - `<apex:detail relatedList="false"/>` ＋ `<apex:relatedList>` で関連リストを取捨選択できる。
> - `<apex:outputField>` は **項目のデータ型に自動適合**（通貨・日付・選択リストを整形）。単独だとラベルなしなので **`<apex:pageBlock>` / `<apex:pageBlockSection>` でラップ**して整える。
> - **反復コンポーネント**は `value`（リスト）を `var`（現在のレコード）に 1 件ずつ割り当てて表・リストを作る。列ヘッダーは項目ラベルから自動生成。
> - `<apex:pageBlockTable>` は Classic スタイル付き、`<apex:dataTable>` / `<apex:dataList>` / `<apex:repeat>` はスタイルなし（`repeat` が最も自由）。

> [!豆知識] `var` の変数名は自由
>
> 反復コンポーネントの `var` 属性に書く名前（`contact` `ct` `a` など）は **開発者が自由に決められるただの一時変数**です。本体の `{!ct.Name}` の `ct` が `var="ct"` と一致してさえいれば動きます。意味の伝わる名前を付けると、後からマークアップを読み返したときに「何のレコードをループしているか」が一目で分かります。
