...
flowchart TD
    Q{"何をしたい？"}
    Q -->|"本格開発・複雑なコード"| DC["開発者コンソール<br/>構文強調・オートコンプリート"]
    Q -->|"ちょっとした修正・お試し"| QF["開発モードの<br/>クイック修正/フッター"]
    Q -->|"ページ設定の変更"| SE["設定エディター<br/>（Lightning 有効化など）"]
    Q -->|"組織横断の本格開発"| VS["VS Code<br/>＋ Salesforce 拡張機能"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class Q hl;
    class DC,QF,SE,VS soft;
```
...
```mermaid
flowchart TD
    P["apex:page<br/>①ページ全体（一番外側）"] --> H["h1：Hello World"]
    P --> PB["apex:pageBlock<br/>②関連項目をまとめる枠"]
    PB --> PBS["apex:pageBlockSection<br/>③折りたためるセクション"]
    PBS --> T["I'm three components deep!<br/>（中身のテキスト）"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class P hl;
    class PB,PBS soft;
...
この単元では、開発者コンソールで Visualforce ページを作成・編集し、属性（`sidebar` など）とコンポーネントを **ネスト（入れ子）** して画面構造を組み立てる方法を学びました。

次の表は、作成方法・主要属性・ネストの親子ルールという、この単元の要点を凝縮したものです。

| 観点 | 要点 |
| --- | --- |
| 作成方法 | **開発者コンソール**（高機能）／クイック修正フッター（お手軽）／設定エディター（設定変更）／**VS Code**（組織横断） |
| 主な属性 | `sidebar` `showHeader` `standardStylesheets` `standardController`（`属性名="値"` 形式） |
| Lightning の挙動 | `sidebar` / `showHeader` は **常に `false`** に上書きされる |
| ネスト | `<apex:pageBlockSection>` は **必ず `<apex:pageBlock>` の内側** に置く（親子ルール） |
| 部品の確認 | 約 150 個のコンポーネントは **「標準コンポーネントの参照」** で都度確認 |

> [!まとめ] この単元の要点
>
> - Visualforce ページは **`<タグ>` ベースのマークアップ**で作り、**固有の URL**（`/apex/ページ名`）でアクセスする。
> - 作成・編集は **開発者コンソール**が最も高機能（構文強調・オートコンプリート・タグペア照合）。
> - **属性**は `属性名="値"` の形でコンポーネントの動作を変える。`sidebar` / `showHeader` は **Lightning では常に `false`**。
> - コンポーネントは **ネスト（入れ子）** でき、`<apex:pageBlockSection>` のように **特定の親の中でしか使えない**親子ルールを持つものがある。
> - 部品の暗記は不要。**「標準コンポーネントの参照」**で属性やネスト可否を確認するのが近道。

> [!豆知識] プレビューは「保存」で動く
>
> 開発者コンソールの [Preview] は、編集内容を **保存した瞬間に自動更新**されます。「コードを変えたのに画面が変わらない」というつまずきの大半は保存忘れが原因。Ctrl+S（保存）を押すクセを付けると、見た目の確認サイクルが一気に速くなります。
