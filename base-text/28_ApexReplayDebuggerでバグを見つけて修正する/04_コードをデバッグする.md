...
このステップでは、コードをリリースしてテストで失敗を再現し、ブレークポイントとチェックポイントを設定して再生対応ログを取得、Replay Debugger でログを再生して変数の不一致からバグを特定し、修正して合格まで導く一連のデバッグ作業を体験しました。

次の表は、デバッグで使った 2 つの目印の違いをまとめたものです（試験頻出）。

| 種類 | 取得できる情報 | 個数の上限 | 有効期限 | 切り替えコマンド |
| --- | --- | --- | --- | --- |
| **ブレークポイント** | その時点の変数値 | **無制限** | （即時利用） | [Debug: Toggle Breakpoint] |
| **チェックポイント** | ヒープダンプ（ローカル・静的・トリガー変数まで） | **最大 5 個** | チェックポイント 30 分／ヒープダンプ 1 日 | [SFDX: Toggle Checkpoint] |

> [!まとめ] このステップの要点
>
> - ソースを組織にリリースし、テストで失敗を**再現**してからデバッグに入る。
> - **ブレークポイントは無制限／チェックポイントは最大 5 個**。チェックポイントはヒープダンプを取得し情報量が多い。
> - チェックポイントは **[Update Checkpoints in Org]** で組織と同期し、**30 分・ヒープダンプ 1 日**で失効する前に再生する。
> - 再生対応ログは **[Turn On Apex Debug Log for Replay Debugger]** で有効化し、**Apex は FINEST**（Visualforce は FINER または FINEST）。
> - 引数 `tickerSymbol`（"CRM"）と `newAcct.TickerSymbol`（"SFDC"）の不一致から代入ミスを特定し、`TickerSymbol = tickerSymbol` に修正して合格させた。

> [!豆知識] 「入力は正しいのに出力が違う」は黄金の手がかり
>
> 今回の決め手は、引数 `tickerSymbol = "CRM"` は正しいのに `newAcct.TickerSymbol = "SFDC"` が違う、という対比でした。デバッグでは「どこまでが正しく、どこから狂うか」の境界を探すのが定石です。境界の直前にある処理（ここでは代入行）が容疑者になります。Replay Debugger なら録画したログを巻き戻して両方の値を同時に見比べられるため、この境界を一瞬で絞り込めます。
