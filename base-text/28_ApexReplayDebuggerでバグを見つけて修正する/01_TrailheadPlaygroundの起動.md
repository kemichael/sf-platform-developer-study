...
このステップでは、デバッグとテストの違いを整理し、デバッグログを「巻き戻し再生」してバグを追う Apex Replay Debugger の正体と、無料の Replay と有料の Interactive の使い分けを押さえました。

次の図は、このプロジェクト全体で繰り返すデバッグサイクルと、再生対応ログに必要なログレベルを俯瞰したものです。

```mermaid
flowchart TD
    E["Apex コードを編集"] --> D["組織にリリース（デプロイ）"]
    D --> R["バグのシナリオを再現<br/>（テスト実行）"]
    R --> L["デバッグログを取得<br/>Apex は FINEST 必須"]
    L --> P["Replay Debugger で再生<br/>変数値を確認"]
    P --> Q{"バグを特定できた？"}
    Q -->|"はい"| FIX["コードを修正"]
    Q -->|"まだ"| P
    FIX --> E
    ...
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class P hl;
    class E,D,R,L,FIX soft;
```

> [!まとめ] このステップの要点
>
> - **テスト**はエラーを見つけて報告する作業、**デバッグ**はその原因を突き止めて修正する作業。
> - **Apex Replay Debugger** は無料で、デバッグログを後から再生して変数値・コールスタックを確認できる。
> - デバッグは「編集 → リリース → 再現 → ログ取得 → 再生」を回す反復プロセス。
> - 再生対応ログのログレベルは **Apex コードなら FINEST**（Visualforce は FINER または FINEST）。
> - 有料の **Apex Interactive Debugger** は実行をリアルタイムに停止できる点が Replay との違い。

> [!豆知識] 「バグ」という言葉と本物の虫 🐞 の由来
>
> プログラムの不具合を「バグ（虫）」と呼ぶのは、1947 年にハーバード大学の計算機 Mark II のリレーに本物の蛾（moth）が挟まり誤動作した逸話が有名です。技術者がその蛾をログに貼り付け「First actual case of bug being found（実際に虫が見つかった最初の事例）」と記録しました。デバッグ（debug）= 虫を取り除く、という言葉はここに通じています。
