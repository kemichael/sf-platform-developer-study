...
この単元は、Agentforce Vibes を「チーム標準に沿った開発パートナー」として使うための4つの構成要素 — チャット・ルール・スキル・アビリティ — の役割分担と、組織コンテキストを与えるほど精度が上がる仕組みを学ぶ回でした。

次の表で、4つのキーワードの違いを一気に整理します。

| 要素 | 一言でいうと | 適用タイミング |
| --- | --- | --- |
| **チャット** | 会話しながら実際に作業する入口 | 開発者が話しかけたとき |
| **ルール（Rules）** | 常に従う永続的なコーディング標準 | **常に**（全生成に自動適用） |
| **スキル（Skills）** | 再利用可能なモジュール化された指示セット | 要求が**説明に一致したときだけ** |
| **アビリティ（Abilities）** | スキル内で実行される具体的な処理 | スキル実行時 |

> [!まとめ] この単元の要点
>
> - **チャット**は活動バーの Agentforce Vibes アイコンから開き、作業中のファイルと組織コンテキストを認識する。
> - **ルール**は「いつも」適用される永続的な指示。作成は `/newrule` または UI。
> - **スキル**は「必要なときだけ」有効化され、トークン効率を高める。中で動く具体処理が**アビリティ**。
> - 命名規則：**Apex クラス＝パスカルケース（`AccountService`）／ LWC ＝ケバブケース（`account-detail`）**。
> - `@` で既存ファイルを参照させると、組織やプロジェクトに合った生成精度が上がる。

> [!豆知識] 「ルールはいつも、スキルは必要なときだけ」を支える節約術
>
> スキルが要求に一致したときだけ読み込まれるのは、AI が一度に扱えるトークン（情報量）の枠を無駄遣いしないためです。常時 100 個のスキル説明を全文読み込むと、肝心のコードや組織情報に割けるスペースが圧迫されます。「関連するものだけロードする」という設計が、結果的に応答の正確さと速さの両方を底上げしています。
