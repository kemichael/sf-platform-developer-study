...
この単元は、MCP（Model Context Protocol）が AI を外部のツール・データにつなぐオープン標準であること、その専用実装である Salesforce DX MCP サーバーが組織のメタデータを自動供給し、手動コピペなしで「組織に合ったコード」を生成できるようにする仕組みを学ぶ回でした。

次の図は、MCP が登場概念をどうつなぐかを俯瞰します。

```mermaid
flowchart TD
    AI["Agentforce Vibes<br/>（AI クライアント）"] -->|"MCP で接続"| MCP["MCP<br/>（オープン標準＝万能翻訳機）"]
    MCP --> DX["Salesforce DX MCP サーバー"]
    MCP --> EXT["追加の MCP サーバー<br/>（DB・外部ツール等）"]
    DX --> ORG["Salesforce 組織<br/>（メタデータ・設定・既存コード）"]
    DX --> AWARE["環境認識を獲得<br/>（手動コピペ不要）"]
    AWARE --> CODE["組織のパターンに沿った<br/>コードを生成"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class MCP hl;
    class CODE,AWARE soft;
```

> [!まとめ] この単元の要点
>
> - **MCP** は AI が**外部のツール・データをセキュアに使えるようにするオープン標準**（速くする・メモリ削減ではない）。
> - **Salesforce DX MCP サーバー**は組織のメタデータ・設定・開発ツールへのアクセスを提供する専用実装。
> - MCP 統合でエージェントが**環境認識を獲得**し、ログやメタデータの**手動コピペが不要**になる。
> - 使い始めの流れ：DX MCP サーバー有効化 → 追加サーバー接続 → 認証・権限設定 → チャット開始（自動で MCP 使用）。
> - 接続前の**認証と権限**の設定は必須。リモート型・Salesforce ホスト型のどちらも利用可。

> [!豆知識] MCP は2024年末に登場したばかりの新しい標準
>
> MCP は Anthropic が2024年11月に公開したオープン標準で、AI と外部ツールをつなぐ「USB-C ポート」によくたとえられます。登場から1年あまりで Salesforce をはじめ多くのベンダーが採用し、Salesforce DX MCP サーバーのように各社が自社環境専用の MCP サーバーを提供する流れが急速に広がりました。試験範囲に入るほど新しいトピックです。
