...
```mermaid
sequenceDiagram
    participant A as Apex コード
    participant DB as データベース
    A->>A: Account acct = new Account（…）
    A->>DB: insert acct;
    DB->>DB: Acme 取引先を作成<br/>Id を自動採番
    DB-->>A: 採番した Id を返す
    A->>A: acct.Id で ID を取得
```
...
```mermaid
flowchart TD
    S(["upsert 実行"]) --> Q["照合キーで<br/>既存レコードを検索"]
    Q --> Z{"一致した件数は？"}
    Z -->|"0 件"| INS["挿入<br/>新規レコードを作成"]
    Z -->|"1 件"| UPD["更新<br/>既存レコードを変更"]
    Z -->|"2 件以上"| ERR["エラー<br/>挿入も更新もされない"]
...
sequenceDiagram
    participant A as Apex コード
    participant DB as データベース
    A->>DB: insert acct（親の取引先）
    DB-->>A: acct.Id を返す
    A->>A: 子の AccountId に acct.Id をセット
    A->>DB: insert mario（子の取引先責任者）
    DB->>DB: 親に紐づく子レコードを作成
```
...
```mermaid
flowchart TD
    S(["トランザクション開始"]) --> A["insert 取引先A：成功"]
    A --> B["insert 取引先B：成功"]
    B --> C["update 取引先責任者：失敗<br/>（入力規則エラー）"]
    C --> Q{"いずれか失敗？"}
    Q -->|"はい"| R["全体をロールバック<br/>取引先 A も B もコミットされない"]
    Q -->|"いいえ"| K["全件コミット"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    class C,R hl;
```
