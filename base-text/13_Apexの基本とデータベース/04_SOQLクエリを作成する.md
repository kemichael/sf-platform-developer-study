...
この単元では、Salesforce のデータベースからレコードを取得する SOQL を学びました。`SELECT ... FROM ... WHERE` の基本形に、`ORDER BY`・`LIMIT`・バインド変数・親子クエリを組み合わせ、ガバナ制限を意識したバルク化までを押さえました。

次の図は、SOQL の句が決まった順序で並ぶことと、関連レコードを2方向にたどれることを俯瞰したものです。

```mermaid
flowchart TD
    SEL["SELECT 項目<br/>必須"] --> FRM["FROM オブジェクト<br/>必須"]
    FRM --> WHR["WHERE 条件<br/>省略可・バインド変数 :var"]
    WHR --> ORD["ORDER BY 並び替え<br/>省略可・ASC / DESC"]
    ORD --> LIM["LIMIT 件数<br/>省略可"]
    LIM --> REL{"関連レコードを<br/>たどる？"}
    REL -->|"親→子"| CH["内部クエリ<br/>（SELECT … FROM Contacts）<br/>リレーション名を指定"]
    REL -->|"子→親"| PA["ドット表記<br/>Account.Name<br/>項目名を 親.項目 で"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class SEL,FRM hl;
    class WHR,ORD,LIM,CH,PA soft;
```

> [!まとめ] この単元の要点
>
> - SOQL は読み取り専用。基本形は **`SELECT 項目 FROM オブジェクト WHERE 条件`**。`SELECT *` は使えず項目を明示する（`FIELDS(STANDARD)` 等は可）。
> - 句の順序は **`SELECT → FROM → WHERE → ORDER BY → LIMIT`** で固定。Apex では `Id` を書かなくても常に返る。
> - **バインド変数（`:変数`）** で Apex の値を差し込める。SOQL インジェクション対策にも有効。
> - 関連レコードは **親→子は内部クエリ（リレーション名）／子→親はドット表記（`Account.Name`）** でたどる。
> - ガバナ制限は **SOQL 100 クエリ・50,000 行**。ループ内クエリを避ける**バルク化**と、メモリ節約の **SOQL for ループ（リスト形式・200件バッチ）** が定石。

> [!豆知識] SOQL は SQL に「似て非なる」言語
>
> SOQL は SQL にそっくりですが、できることは大きく違います。SQL の `INSERT`・`UPDATE`・`DELETE` は SOQL には存在せず（それは DML の仕事）、`JOIN` も書けません。代わりにリレーション項目を使った親子クエリでオブジェクトをまたぎます。「読み取り専用・JOIN なし・リレーションでたどる」が SOQL の個性です。読みは公式に「ソークル（SO-kul）」とされています。
