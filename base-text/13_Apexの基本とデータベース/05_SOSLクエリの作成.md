...
この単元では、複数のオブジェクトを横断してテキスト検索する SOSL を学びました。SOQL との違い（単語一致 vs 完全一致、複数 vs 単一オブジェクト）、`FIND ... RETURNING ...` の構文、囲み記号の使い分け、戻り値が「リストのリスト」になることを押さえました。

次の表は、SOQL と SOSL を一目で対比できるようまとめたものです。
...
| 対象オブジェクト | **1 つ** | **複数を横断** |
| 一致方法 | **完全一致**（既定） | **単語一致** |
| 戻り値（Apex） | `List<sObject>` | `List<List<sObject>>`（リストのリスト） |
| 囲み（Apex / エディター） | 角括弧 `[ ]` で囲む | Apex は `' '`／エディターは `{ }` |
| ガバナ制限 | 100 クエリ・50,000 行 | 20 クエリ・2,000 行 |

> [!まとめ] この単元の要点
>
> - SOSL は **`FIND`** で始まるテキスト検索言語。**複数オブジェクトを横断**し**単語一致**で検索する。
> - クエリの囲みは **Apex は `'シングルクオート'`／クエリエディター・API は `{中括弧}`**（頻出の引っかけ）。
> - 戻り値は **`List<List<sObject>>`（リストのリスト）**。`searchList[0]` から **`RETURNING` の順**にオブジェクトが入り、`(Account[])` のようにキャストして取り出す。
> - `IN ALL FIELDS` などの **SearchGroup** で項目範囲を、`RETURNING` の各 sObject 括弧内で **WHERE / ORDER BY / LIMIT** を指定できる。
> - ガバナ制限は **SOSL 20 クエリ・1 クエリ 2,000 行**（SOQL の 100/50,000 と混同しない）。検索インデックスの遅延に注意。

> [!豆知識] SOSL の検索エンジンは Apache Lucene 由来
>
> SOSL の高速な全文検索は、オープンソースの全文検索ライブラリ Apache Lucene と同系統の仕組みで支えられています。レコードを保存すると裏側で「検索インデックス」が作られ、そのインデックスに対して単語単位で照合するため、`(415)555-1212` という電話番号も `415`・`555`・`1212` の単語に分解されてヒットします。インデックス更新には数秒のラグがあるため、テストコードでは `Test.setFixedSearchResults()` で結果を固定するのが定番です。
