...
```text
  ┌──────────────────────────┐        ┌──────────────────────────┐
  │  データベース（Salesforce） │        │  Apex のメモリ            │
  │  取引先レコード「Acme」     │◀─取得─▶│  Account 型の sObject 変数 │
  │  Id / 名前 / 電話 / 従業員数 │  挿入  │  acct.Name = 'Acme' …     │
  └──────────────────────────┘        └──────────────────────────┘
```
...
カスタムオブジェクトは **API 参照名** を使います。たとえば Merchandise カスタムオブジェクトは `Merchandise__c` sObject に対応します。
...
```text
  汎用 sObject 変数（sObject 型）
  ┌───────────────────────────┐
  │ sObject sobj;             │
  └───────────────────────────┘
       ▲          ▲          ▲
       │ OK       │ OK       │ OK
   Account     Book__c     Contact      … 何でも参照できる

  特定 sObject 変数（Account 型）
  ┌───────────────────────────┐
  │ Account acct;             │
  └───────────────────────────┘
       ▲          ✕          ✕
       │ OK       │ NG       │ NG
   Account     Book__c     Contact      … 同じ型しか参照できない
```
