...
この単元では、Salesforce の1件のレコードを Apex のメモリ内で扱う「sObject」の作り方・項目の設定方法・API 参照名のルール、そして汎用 sObject と特定 sObject の使い分けとキャストを学びました。

次の表は、sObject を扱ううえで押さえるべき「型・項目・キャスト」の要点をまとめたものです。

| 観点 | 特定 sObject | 汎用 sObject |
| --- | --- | --- |
| 宣言例 | `Account acct;` | `sObject sobj;` |
| 参照できるレコード | 宣言した型と**同じ型のみ** | 標準・カスタム問わず**何でも** |
| ドット表記での項目アクセス | **できる**（`acct.Name`） | **できない**（要キャスト or `get()`/`put()`） |
| 主な用途 | 型が確定している通常処理 | 型が不明な汎用メソッド |

> [!まとめ] この単元の要点
>
> - すべての Salesforce レコードは Apex では **sObject** として表現され、`new` で作成する。
> - 項目設定は **コンストラクター**（`new Account(Name='Acme')`）と **ドット表記**（`acct.Name='Acme'`）の2通り。取引先の必須項目は **Name だけ**。
> - 項目参照は表示ラベルではなく **API 参照名**。カスタムは `__c`、カスタムリレーションは `__r`、標準はサフィックスなし。
> - **汎用 sObject** は何でも入る／**特定 sObject** は同型のみ。汎用はキャストするとドット表記が使え、しない場合は `get()`/`put()`。
> - sObject はメモリ上の入れ物にすぎず、保存には **DML**、取得には **SOQL** が必要（次の単元）。

> [!豆知識] 「sObject」の s は小文字が正式
>
> sObject は「Salesforce Object」の略ですが、先頭の s は慣例的に小文字で書きます。Apex は大文字小文字を区別しないため `SObject` と書いても動きますが、Salesforce の公式ドキュメントやサンプルコードでは一貫して `sObject` 表記が使われています。型名としては `SObject` クラスが正式に存在し、すべての具体的な sObject 型（Account など）の親型になっています。
