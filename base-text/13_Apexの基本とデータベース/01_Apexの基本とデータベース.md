...
Apex は Salesforce のサーバー上に保存・コンパイル・実行され、データベースと密接に統合されます。クライアントからの要求に対し、サーバー側で Apex が動いてデータベースを操作し、結果を返す流れです。

```mermaid
flowchart LR
    C(["クライアント<br/>ボタン操作・レコード更新"]) -->|"要求"| S["Salesforce サーバー<br/>Apex を実行"]
    S -->|"DML / SOQL"| DB[("データベース")]
    DB -->|"結果"| S
    S -->|"応答"| C
...
Apex クラスの利点はコードの再利用です。クラスメソッドはトリガーやほかのクラスからコールできます。以降では、サンプルクラスを組織に保存し、メールを送信してデバッグログを調べます。この単元のハンズオンは、次の流れで進みます。

```mermaid
flowchart TD
    S(["開始"]) --> A["開発者コンソールで<br/>EmailManager クラスを保存<br/>（コンパイル）"]
    A --> B["匿名 Apex で<br/>sendMail を実行"]
    B --> C["メール送信<br/>System.debug でログ出力"]
    C --> D["デバッグログを確認<br/>Debug Only で絞り込み"]
    D --> E["sendMail を static に変更<br/>クラス名から直接コール"]
    E --> F(["完了"])
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    class A,B hl;
```
...
```mermaid
flowchart TD
    Q{"メンバー変数を使う？"} -->|"使う"| I["インスタンスメソッド<br/>EmailManager em = new EmailManager;<br/>em.sendMail（…）"]
    Q -->|"使わない"| St["静的メソッド static<br/>EmailManager.sendMail（…）<br/>new 不要"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class St hl;
    class I soft;
...
この単元では、Salesforce 専用言語 Apex の正体（Java に似た構文・サーバー上で保存／コンパイル／実行・マルチテナント制限）と、クラスを保存して匿名 Apex で実行し、デバッグログで結果を確認するまでの開発の基本サイクルを学びました。

次の図は、Apex を書いてから動作を確認するまでの一連の流れを俯瞰したものです。

```mermaid
flowchart TD
    W["クラスを記述<br/>開発者コンソール / VS Code"] --> S["保存＝コンパイル<br/>サーバー上で型チェック"]
    S --> R{"呼び出し方は？"}
    R -->|"インスタンス"| I["new でインスタンス生成<br/>em.sendMail（…）"]
    R -->|"static"| St["クラス名から直接<br/>EmailManager.sendMail（…）"]
    I --> X["実行<br/>匿名 Apex / トリガー"]
    St --> X
    X --> L["デバッグログを確認<br/>System.debug・Debug Only"]
    L --> G["ガバナ制限の範囲内か<br/>バルク化で制限を回避"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class W hl;
    class S,I,St,X,L,G soft;
```

> [!まとめ] この単元の要点
>
> - Apex は **Java に似た構文・サーバー上で保存／コンパイル／実行・強い型付け・マルチテナント**の言語。大文字小文字は区別しない。
> - コレクションは **List（順序付き・重複可）／ Set（重複なし）／ Map（キーと値）** の3種類。リストのインデックスは **0 から**始まる。
> - クラスは保存時にコンパイルされ、**匿名 Apex** でその場限りの実行ができる。`System.debug()` の出力はデバッグログで確認する。
> - メンバー変数を使わないメソッドは **`static`** にすると `new` 不要でクラス名から直接呼べる。
> - **ガバナ制限**（SOQL 100・DML 150 など）に注意し、ループ内でクエリ／DML を呼ばない**バルク化**が鉄則。本番デプロイには**コードカバー率 75% 以上**が必要。

> [!豆知識] Apex は Salesforce の最古参テクノロジーのひとつ
>
> Apex は 2007 年に登場した、クラウド上で動く商用プログラミング言語の草分け的存在です。言語名「Apex」には「頂点」という意味があり、Salesforce が全テナントのコードを自動でアップグレード前に再実行（テスト）するため、後方互換性を保ったまま長年進化を続けています。あなたが書いた Apex も「API バージョン」で固定保存され、将来の更新でも動き続けます。
