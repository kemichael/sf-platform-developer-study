...
この単元では、Apex でデータを扱う3本柱（照会の SOQL・全文検索の SOSL・書き込みの DML）と、それに付随する「ガバナ制限」「バルク化」「例外処理」を整理しました。

次の表は、SOQL / SOSL / DML の役割と使いどころを一目で見分けるための早見表です。

| 言語・操作 | 役割 | キーワード | こんなときに使う |
| --- | --- | --- | --- |
| **SOQL** | 照会（読み取り） | `SELECT ... FROM ... WHERE` | 条件が明確な**1オブジェクト**から取得 |
| **SOSL** | 全文検索（読み取り） | `FIND ... IN ... RETURNING` | **複数オブジェクト横断**のテキスト検索 |
| **DML** | 書き込み | `insert/update/upsert/delete/...` | レコードの作成・更新・削除 |

> [!まとめ] この単元の要点
>
> - **SOQL=照会／SOSL=全文検索／DML=書き込み** の役割を区別する（取り違えが頻出）。
> - **ガバナ制限**の代表値は **SOQL 100 回・DML 150 回**。超えると実行時例外で停止。
> - 最重要パターンは**バルク化**＝**for ループ内で SOQL/DML をしない**。ループ外でまとめて実行する。
> - エラーは **`try / catch / finally`** で処理し、業務固有の異常は**カスタム例外**で表す。
> - catch で握りつぶさず、最低限ログ出力やエラー処理を入れる。

> [!豆知識] SOSL の「SO」は SOQL と同じ、最後の1文字だけが違う
>
> **SOQL**（Salesforce Object Query Language）と **SOSL**（Salesforce Object Search Language）は最後が Query か Search かの違いだけ。「Q＝Query＝条件で1オブジェクトを問い合わせ」「S＝Search＝複数オブジェクトを横断検索」と、末尾の頭文字で役割を思い出せます。読み方も「ソークル」「ソッスル」と似ているので、試験では文章の「複数オブジェクト」「全文検索」というキーワードで SOSL を見抜くのが確実です。
