...
この単元は試験対策モジュールの入り口として、認定 Platform デベロッパー試験の「4セクションの配点」「受験ポリシー（協力 OK／不正 NG の線引き）」「年1回の資格更新」という、学習を始める前に必ず押さえる枠組みを確認しました。

次の図は、試験の4セクションと配点、優先して固めるべき領域を1枚で俯瞰したものです。
...
> [!まとめ] この単元の要点
>
> - 試験は **4 セクション**で、配点は **27 / 28 / 25 / 20（合計 100%）**。最大は「プロセスの自動化とロジック（28%）」。
> - 配点上位の **①開発者の基本事項（27%）＋②自動化とロジック（28%）= 55%** をまず固めるのが合格への近道。
> - 学習は **Trailhead の承認済み資料**で行い、**Trailblazer Community** での協力は推奨（準備段階の協力は OK）。
> - **試験問題・解答の共有や受験中の手助けは禁止**。違反すると資格取り消し等の措置がある。
> - 資格維持には **年1回のリリース更新モジュール完了**が必須。怠ると失効する。

> [!豆知識] 配点は「割り切れない数字」だから覚えやすい
>
> 4セクションの配点 27 / 28 / 25 / 20 は、足すとちょうど 100% になります。語呂で「ニーナナ・ニーハチ・ニーゴー・ニーゼロ」とリズムで覚えると、本試験の「各セクションの割合は？」という設問に即答できます。最大配点が「自動化とロジック（28%）」である点だけは取り違えないようにしましょう。
