...
```mermaid
flowchart TD
    Sel["selector（コンテナ・親）"] --> List["list"]
    Sel --> Detail["detail<br/>選択された自転車の詳細を表示"]
    List --> T1["tile"]
    List --> T2["tile"]
    List --> T3["tile …"]
...
```mermaid
flowchart TD
    Parent["親コンポーネント"] -->|"プロパティ（下へ）<br/>@api で値を渡す"| Child["子コンポーネント"]
    Child -->|"イベント（上へ）<br/>dispatchEvent で発火"| Parent
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class Parent hl;
    class Child soft;
...
sequenceDiagram
    ...
    Note over Tile: クリック発生
    Tile->>List: ① CustomEvent「tileclick」を dispatch（上へ）
    Note over List: ontileclick → handleTileClick
    List->>Sel: ② CustomEvent「productselected」を dispatch（上へ）
    Note over Sel: onproductselected → handleProductSelected
    Sel->>Detail: ③ product-id={selectedProductId} で<br/>プロパティを渡す（下へ）
    Note over Detail: set/get productId → product を更新 → 詳細を表示
```
