...
flowchart TD
    Q1{"isExposed が true？"}
    Q1 -->|"いいえ"| NG["ビルダーに表示されない"]
    Q1 -->|"はい"| Q2{"target を1つ以上定義？"}
    Q2 -->|"いいえ"| NG
    Q2 -->|"はい"| OK(["ビルダーに表示される<br/>ページに配置可能"])
...
```mermaid
flowchart TD
    A["VS Code でファイル作成<br/>bikeCard.html / .js / .js-meta.xml"] -->|"SFDX: Deploy This Source to Org"| B["Salesforce 組織にリリース"]
    B -->|"Lightning アプリケーションビルダー"| C["アプリケーションページにドラッグ"]
    C -->|"保存 → 有効化"| D(["UI に Bike Card コンポーネントが表示"])
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class A hl;
    class B,C soft;
