...
> ```text
>  正常時：
>   クライアント ──「studentApi=https://192.168.0.1/student」──▶ サーバー
>                                                                  │
>                                                  GET /student ─────┘─▶ 社内の生徒サービス（正規）
>
>  攻撃時：
>   攻撃者がリクエストを傍受し宛先を差し替え
>   クライアント ──「studentApi=http://169.254.169.254/...」──▶ サーバー
>                                                                  │
>                       サーバーが信じて GET ─────────────────────┘─▶ クラウドのメタデータ
>                                                                       エンドポイント（本来非公開！）
>                                                                       └─▶ 認証情報・構成データが漏洩
> ```
