...
この単元では、サーバー自身を踏み台にして任意の宛先（特に社内サービスやクラウドのメタデータエンドポイント）へリクエストを送らせる SSRF 攻撃と、その防止策を学びました。CSRF が「クライアント」を、SSRF が「サーバー」を悪用する点が最大の違いです。

次の図は、SSRF 対策の柱を「送信先 URL を信頼済みに絞り込む」流れで俯瞰したものです。

```mermaid
flowchart TD
    S(["送信先 URL を受け取る"]) --> V["入力の検証・サニタイズ<br/>file:// や内部アドレスを弾く"]
    V --> P["URL 解析ライブラリで<br/>スキーマ・ホスト・ポートを抽出"]
    P --> A{"許可リストの<br/>宛先か？"}
    A -->|"いいえ（内部アドレス等）"| REJ["拒否<br/>SSRF をブロック ✓"]
    A -->|"はい（信頼済み）"| ACC["リクエスト送信を許可<br/>リダイレクトは無効化"]
    REJ --> N["ネットワーク分離で<br/>万一の影響範囲も限定"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class S hl;
    class V,P,REJ,ACC,N soft;
```

> [!まとめ] この単元の要点
>
> - **SSRF** は、攻撃者がサーバーを騙し、サーバー自身に任意の宛先（特に内部リソース・メタデータエンドポイント）へリクエストを送らせる攻撃。
> - **CSRF はクライアント（ブラウザ）、SSRF はサーバー**が悪用される点が最大の違い。
> - 対策の柱：**①入力の検証・サニタイズ ②許可リスト（宛先制限・リダイレクト無効化）③URL 解析ライブラリ ④ネットワーク分離**。
> - Lightning では **GET を避け POST/PUT、Origin ヘッダー検証、Anti-SSRF トークン、ページ読み込みで自動 DML しない**。
> - 主要な標的はクラウドのメタデータエンドポイント（`169.254.169.254` など）。

> [!豆知識] 169.254.169.254 は「リンクローカル」の特別なアドレス
>
> SSRF で頻出する `169.254.169.254` は、主要クラウド（AWS・GCP・Azure）が共通で採用するメタデータサービスの IP で、`169.254.0.0/16` という「リンクローカルアドレス」帯に属します。この帯はルーターを越えて転送されず、同一ホスト・同一リンク内からしか到達できません。だからこそ「外部の攻撃者は直接届かないが、踏み台にされたサーバーからは届く」という SSRF の典型シナリオが成立します。許可リストでこの帯を明示的に拒否するのが定石です。
