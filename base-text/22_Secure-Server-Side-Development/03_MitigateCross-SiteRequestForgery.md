...
>
> ```text
>  ① あなた（管理者）が School District 組織にログイン（セッション有効）
>         │
>         ▼
>  ② 別の悪意あるサイトを閲覧し、リンクをクリック
>         │
>         ▼
>  ③ 悪意あるサイトがブラウザに、信頼済みサイトへの GET を送らせる
>     GET https://schooldistrict.app/promote?UserId=123
>         │  ← あなたのセッション Cookie が自動で付く！
>         ▼
>  ④ サーバーは「正規の管理者からの正当なリクエスト」と誤認
>         │
>         ▼
>  ⑤ 生徒123 が優等生名簿に昇格（あなたは意図していない！）
> ```
...
```text
  ┌──────────────────── トークン無しの脆弱なリクエスト ────────────────────┐
  │ GET /promote?UserId=123                                              │
  │   → 攻撃者はパラメータ名と値さえ知れば偽造できる ✗                    │
  └──────────────────────────────────────────────────────────────────────┘

  ┌──────────────────── トークン有りの安全なリクエスト ────────────────────┐
  │ POST /promote   body: UserId=123 & token=9f2c...（毎回変わる秘密値）   │
  │   → 攻撃者はトークンを推測できず偽造不能。サーバーが検証して拒否 ✓     │
  └──────────────────────────────────────────────────────────────────────┘
```
