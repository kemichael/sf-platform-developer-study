...
この単元では、ログイン中ユーザーのブラウザを悪用して信頼済みサイトに意図しない操作を実行させる CSRF 攻撃の仕組みと、コードレベル・組織レベルの防止策を学びました。

次の図は、CSRF を防ぐ4つの柱を一枚にまとめたものです。

```mermaid
flowchart TD
    S(["CSRF を防ぐ"]) --> G["状態変更に GET を使わない<br/>→ POST / PUT を使う"]
    S --> T["CSRF トークンを検証<br/>推測困難・一意・サーバーで検証"]
    S --> O["Origin ヘッダーを検証<br/>別サイト由来を拒否"]
    S --> D["ページ読み込みで DML しない<br/>クリック等の明示操作をトリガーに"]
    G --> SF["Salesforce 標準で<br/>CSRF トークンを付与<br/>（頼り切らない）"]
    T --> SF
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class S hl;
    class G,T,O,D,SF soft;
```

> [!まとめ] この単元の要点
>
> - **CSRF** は、ログイン中ユーザーの**ブラウザ**を悪用し、信頼済みサイトへ本人の意図しない状態変更を実行させる攻撃。
> - 攻撃成立には「**被害者が認証済み**」かつ「**攻撃者がパラメータ名と値を用意できる**」ことが必要。
> - 防止の4本柱：**①状態変更に GET を使わない（POST/PUT）②推測困難な CSRF トークンを検証 ③Origin ヘッダー検証 ④ページ読み込みで DML しない**。
> - Lightning では `doInit`（`onInit`）など**読み込みイベントでの自動 DML が脆弱**。状態変更は**クリック等の明示操作**をトリガーにする。
> - Salesforce は標準で CSRF トークンを付与するが、**コード設計の注意は依然必要**。

> [!豆知識] Origin ヘッダーは JavaScript から書き換えられない
>
> CSRF 対策で Origin ヘッダーが信頼できるのは、これがブラウザが自動付与する「禁止ヘッダー（Forbidden header）」だからです。`fetch` や `XMLHttpRequest` の JavaScript からは Origin を任意の値に改ざんできない仕様になっており、攻撃者のサイトから送られたリクエストには必ず攻撃者サイトの Origin が刻まれます。だからサーバー側で「想定外の Origin を拒否」するだけで、別サイト由来の偽造リクエストをはじけるわけです。
