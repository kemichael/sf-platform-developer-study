...
```mermaid
flowchart TB
    subgraph Classic["Salesforce Classic"]
        VF1["Visualforce ページ<br/>ページ・要求・環境のすべてを「所有」<br/>＝ VF 自身がコンテナ"]
    end
    subgraph LEX["Lightning Experience（親コンテナ）"]
        subgraph Frame["iframe"]
            VF2["Visualforce ページ（子）<br/>iframe の中に閉じ込められる"]
        end
    end
    Classic -.->|"実行環境が異なる"| LEX
...
`window.postMessage` でフレーム境界を越えるやり取りを図にすると次のとおりです。

```mermaid
sequenceDiagram
    participant VF as Visualforce ページ（子・iframe）
    participant LEX as Lightning Experience（親）
    VF->>LEX: postMessage でメッセージを送信
    LEX->>LEX: 受信コードでメッセージを処理
    LEX-->>VF: 必要なら postMessage で応答を返す
    Note over VF,LEX: 直接 DOM/変数には触れず<br/>メッセージのやり取りで安全に連携
```

```mermaid
flowchart LR
    BAD["iframe（VF）"] -->|"window.parent.location を直接操作"| BADP["親（LEX）"]
    BADP --> BADR["同一オリジンポリシー違反<br/>（悪い例）"]
    GOOD["iframe（VF）"] -->|"sforce.one / postMessage"| GOODP["親（LEX）"]
    GOODP --> GOODR["安全に連携<br/>（良い例）"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class BADR hl;
    class GOODR soft;
