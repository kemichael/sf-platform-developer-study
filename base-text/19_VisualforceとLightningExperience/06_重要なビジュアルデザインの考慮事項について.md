...
この単元では、Visualforce ページがデフォルトで Classic の見た目のまま LEX に表示されること、そして LEX らしい外観にする唯一の推奨方法である SLDS の使い方を学びました。

次の図は、ページのスタイルをどう決めるかの判断と、SLDS を適用する 2 通りの方法を俯瞰したものです。

```mermaid
flowchart TD
    S(["VF ページのスタイルを決める"]) --> Q1{"LEX らしい<br/>見た目が必要？"}
    Q1 -->|"いいえ"| KEEP(["Classic の見た目のまま使う<br/>（書き換え不要）"])
    Q1 -->|"はい"| Q2{"新規ページ？"}
    Q2 -->|"はい（推奨）"| SLDS["SLDS で組む"]
    Q2 -->|"いいえ（既存）"| NR(["後付け SLDS／CSS は<br/>技術上可能だが非推奨"])
    SLDS --> W1["方法1：apex:slds タグを追加<br/>（アップロード不要・最も簡単）"]
    SLDS --> W2["方法2：SLDS を静的リソース化し<br/>apex:stylesheet で参照"]
    classDef hl fill:#0176D3,stroke:#032D60,color:#fff;
    classDef soft fill:#E8F2FC,stroke:#0176D3,color:#032D60;
    class S hl;
    class KEEP,SLDS,W1,W2 soft;
```

> [!まとめ] この単元の要点
>
> - Visualforce ページはデフォルトで **Salesforce Classic の見た目** のまま LEX に表示される。
> - コンポーネント単位のスタイル変更は **`style`（直接指定）** と **`styleClass`（クラス指定）** の 2 通り。
> - 共通スタイルは **静的リソース ＋ `<apex:stylesheet>`** で追加するのが推奨。
> - LEX に合わせる唯一の推奨方法は **SLDS** で、適用は **`<apex:slds />`** が最も簡単（容量を消費しない）。
> - SLDS に含まれるのは **CSS フレームワーク・アイコン・Salesforce Sans フォント** の 3 要素。組み込みコンポーネントの HTML 構造・ID・クラスに依存しない。

> [!豆知識] `<apex:slds />` が静的リソース 250 MB を節約する理由
>
> SLDS の CSS 一式は数 MB あり、自分で静的リソースとしてアップロードすると組織全体で 250 MB しかない保存容量を圧迫します。`<apex:slds />` タグは Salesforce 側がホストする SLDS を直接参照するため、自組織の容量を一切消費しません。たった 1 行で「最新版に自動追従しつつ容量も節約」という二重のメリットが得られるのが、このタグが推奨される理由です。
