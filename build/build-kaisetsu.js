 ...
 * 解説集ページのビルド
 *
 * build/exam-data.json（模擬試験の全セット）を学習用の閲覧ページとして、
 * 共有 CSS・marked / Prism ライブラリ・専用スクリプトとともに
 * 自己完結した kaisetsu.html に出力する。
 *
 * 問題・解説の修正は build/exam-data.json を編集してから本スクリプトと
 * build-exam.js（模擬試験ページ）の両方を再実行する。
 */
const fs = require('fs');
...
const OUT = path.join(ROOT, 'kaisetsu.html');
...
const appJs = fs.readFileSync(path.join(__dirname, 'kaisetsu-app.js'), 'utf8');
const template = fs.readFileSync(path.join(__dirname, 'kaisetsu-template.html'), 'utf8');
