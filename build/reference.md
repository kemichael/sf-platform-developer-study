...
```sql
SELECT Name, Phone, NumberOfEmployees   -- 取得項目を必ず列挙（カンマ区切り）
FROM Account                            -- 対象は 1 オブジェクト
WHERE NumberOfEmployees > 25            -- 絞り込み条件（任意）
ORDER BY Name                           -- 並び替え（既定は昇順 ASC）
LIMIT 10                                -- 返す最大件数（任意）
```
...
WHERE (Name = 'SFDC Computing'                 -- 文字列はシングルクォートで囲む
       OR (NumberOfEmployees > 25              -- 括弧で AND / OR の優先順位を明示
           AND BillingCity = 'Los Angeles'))   -- 大文字小文字は区別しない比較
```
...
SELECT FirstName, LastName,
       Account.Name,          -- 親をドットでたどる（標準リレーションはそのまま）
       Account.Owner.Name     -- さらに親の親へ。子→親は最大 5 レベル
FROM Contact
```
...
```sql
SELECT Name,
       (SELECT FirstName, LastName FROM Contacts)   -- 子側 FROM は複数形のリレーション名
FROM Account
```
...
List<Contact> cons =
    [SELECT Id FROM Contact
     WHERE Department = :dept       -- 単一値は :変数 で差し込む
       AND AccountId IN :ids];      -- Set / List はそのまま IN に渡せる
```
...
String q = 'SELECT Id, Name FROM Account WHERE Name = \''
         + String.escapeSingleQuotes(userInput)   -- ユーザー入力は必ずエスケープ
         + '\'';
List<Account> rows = Database.query(q);            -- 文字列を実行時にクエリ化
```
...
```sql
FIND {jos* AND "data cloud"}    -- jos* = jos で始まる語、"…" = 語句をそのまま検索、AND で両方一致
  IN NAME FIELDS                -- 検索範囲を名前項目に限定
  RETURNING Lead(Name)          -- ヒットした Lead の Name を返す
```
...
for (Integer i = 0; i < 200; i++) {
    toInsert.add(new Contact(LastName = 'Test ' + i));  // ループ内では「溜める」だけ
}
insert toInsert;          // DML はループの外で 1 回だけ（200 件でも DML は 1 回）
```
...
public with sharing class EmailManager {
    ...
    public void sendMail(String address, String subject, String body) {
        // void は戻り値なし。引数は「型 名前」を順に並べる
    }
}
```

> [!ポイント] static と インスタンスメソッドの違い
>
> `static` メソッドはインスタンス化せず `EmailManager.calcTaxIncluded(100)` のようにクラス名から直接呼ぶ。`static` が付かないメソッドは `new` したインスタンス経由（`new EmailManager().sendMail(...)`）で呼ぶ。
...
```apex
//        ① トリガー名     ② 対象オブジェクト  ③ 起動イベント（複数可・カンマ区切り）
trigger AccountTrigger on Account (before insert, after insert, after update) {
    // ここに処理。どのイベントで起動したかは Trigger.isBefore / isInsert 等で判定する
}
```
...
```apex
@IsTest                                   // テストクラスには必須。本番カバレッジに含まれない
...
public class ContactsListController {
    // getter メソッド：getXxx() の Xxx 部分が式 {! xxx } の名前になる（先頭は小文字）
    public List<Contact> getContacts() {            // ページ側では {! contacts }
        return [SELECT Id, Name FROM Contact LIMIT 10];
    }
    // アクションメソッド：戻り値 PageReference でページ遷移、null で同じページに留まる
...
```html
<!-- 単一ファイル：$Resource.リソース名 で直接参照 -->
<apex:includeScript value="{! $Resource.jQuery }"/>
<!-- ZIP 内のファイル：URLFOR(リソース名, 'ZIP 内のパス') でたどる -->
...
```html
<aura:component controller="MyContactListController"   <!-- Apex コントローラーを紐付け -->
                implements="flexipage:availableForRecordHome,force:hasRecordId"  <!-- 配置先インターフェース -->
                access="global">                       <!-- 名前空間外からの利用を許可 -->
    <!-- 属性＝コンポーネントの状態。type と name が必須 -->
    <aura:attribute name="contacts" type="Contact[]" />
    <aura:attribute name="recordId" type="Id" />

    <!-- init ハンドラー＝LWC の connectedCallback 相当。生成時に c.doInit を実行 -->
    <aura:handler name="init" value="{!this}" action="{!c.doInit}" />

    <!-- 繰り返し（LWC の for:each 相当）。items に配列、var に各要素名 -->
    <aura:iteration items="{!v.contacts}" var="con">
        <p>{!con.Name}</p>                             <!-- {!v.属性} で値を参照 -->
    </aura:iteration>

    <!-- 条件付き表示（LWC の lwc:if 相当） -->
